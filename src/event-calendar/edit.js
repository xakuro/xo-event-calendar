...
export default function Edit( { attributes, setAttributes } ) {
	const categories = useSelect( ( select ) => {
		return select( 'core' ).getEntityRecords( 'taxonomy', 'xo_event_cat' );
	}, [] );
	...
	const inspectorControls = (
		<InspectorControls>
			...
			<PanelBody
				title={ __( 'Event settings', 'xo-event-calendar' ) }
				initialOpen={ false }
			>
				<RadioControl
					selected={ attributes.event }
					options={ [
						{
							label: __( 'Do not display', 'xo-event-calendar' ),
							value: 'off',
						},
						{
							label: __( 'Display', 'xo-event-calendar' ),
							value: 'on',
						},
						{
							label: __(
				...
				{ 'selected' == attributes.event && (
					<SelectControl
						label={ __( 'Categories', 'xo-event-calendar' ) }
						multiple
						...
						onChange={ ( value ) => {
							setAttributes( {
								categories: value ? value.join( ' ' ) : '',
							} );
						} }
						value={ attributes.categories.split( ' ' ) }
						help={ __(
	...
	return (
		<div { ...useBlockProps() }>
			...
			<Disabled>
				<ServerSideRender
					block="xo-event-calendar/event-calendar"
